...
def render():
    ...
    # ZXZ Euler angles
    global thAlpha, thBeta, thGamma

    zGamma = np.radians(thGamma)
    xBeta = np.radians(thBeta)
    zAlpha = np.radians(thAlpha)
    M = np.identity(4)

    RzGamma = np.array([[np.cos(zGamma), -np.sin(zGamma), 0],
                        [np.sin(zGamma), np.cos(zGamma), 0],
                        [0,0,1]])
    RxBeta = np.array([[1, 0, 0],
                       [0, np.cos(xBeta), -np.sin(xBeta)],
                       [0, np.sin(xBeta), np.cos(xBeta)]])
    RzAlpha = np.array([[np.cos(zAlpha), -np.sin(zAlpha), 0],
                        [np.sin(zAlpha), np.cos(zAlpha), 0],
                        [0,0,1]])
    M[:3,:3] = RzAlpha @ RxBeta @ RzGamma
    glMultMatrixf(M.T)
...
def key_callback(window, key, scancode, action, mods):
    global gCamAng, gCamHeight
    global thAlpha, thBeta, thGamma, initFlag
    ...
    if action==glfw.PRESS or action==glfw.REPEAT:
        ...
        # Rotate along X-axis of the new frame by beta
        elif key==glfw.KEY_S:
            thBeta += 10
        elif key==glfw.KEY_X:
            thBeta -= 10
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480, 480,'CG_weekly_practice_08_2016024875', None,None)
    if not window:
        glfw.terminate()
        return
