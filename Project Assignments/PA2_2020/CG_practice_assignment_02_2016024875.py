...
# Programmaing Assignment #2 (Cow roller coaster)
...
animStartTime=0
isTimeInit=False
cowRollingPos=np.zeros((4,4))

# picking up the 6 locations
pickCount=-1
pickLoc = []
...
def display():
    global cameraIndex, cow2wld, pickCount, pickLoc, animStartTime, isTimeInit, cowRollingPos
    ...
    if -1 < pickCount and pickCount < 6:
        for idx in pickLoc:
            drawCow(idx, True)
        ...
        if 0<=t and t<6 and animTime < 18:
            animTime=float(animTime)-int(animTime)
            if 0<=t and t<1:
                splinePos=getCatmullRom(animTime, pickLoc[5], pickLoc[0], pickLoc[1], pickLoc[2])
            elif 1<=t and t<2:
                splinePos=getCatmullRom(animTime, pickLoc[0], pickLoc[1], pickLoc[2], pickLoc[3])
            elif 2<=t and t<3:
                splinePos=getCatmullRom(animTime, pickLoc[1], pickLoc[2], pickLoc[3], pickLoc[4])
            elif 3<=t and t<4:
                splinePos=getCatmullRom(animTime, pickLoc[2], pickLoc[3], pickLoc[4], pickLoc[5])
            elif 4<=t and t<5:
                splinePos=getCatmullRom(animTime, pickLoc[3], pickLoc[4], pickLoc[5], pickLoc[0])
            elif 5<=t and t<6:
                splinePos=getCatmullRom(animTime, pickLoc[4], pickLoc[5], pickLoc[0], pickLoc[1])
        
        else:
            cow2wld=cowRollingPos
            pickCount=-1
            pickLoc.clear()
            isTimeInit=False
            glFlush()
        ...
        getCowDirection(getTranslation(cowRollingPos), getTranslation(splinePos))
        setTranslation(cowRollingPos, getTranslation(splinePos))
        drawCow(cowRollingPos, False)
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, pickCount, pickLoc, cow2wld
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            ...
            if cursorOnCowBoundingBox:
                if pickCount==-1:
                    pickCount+=1
                elif -1<pickCount and pickCount<6:
                    pickCount+=1
                    pickLoc.append(cow2wld.copy())
                    if pickCount==6:
                        isDrag=0
            else:
                isDrag=0
